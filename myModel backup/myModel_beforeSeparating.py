...
class DCGAN(ABC,object):
    ...
    def __init__(self, sess, flags, 
        ...
        if self.resize : 
            ...
            self.resize_height = flags.output_height
            self.resize_width = flags.output_width
    ...
    def build_model(self):
        ...
        if self.crop:
            ...
            image_dims = [self.resize_height, self.resize_width, self.clr_dim]
        else:
...
class baseDCGan(DCGAN):
    ...
    def train(self, config):
        ...
        batch_idxs = min(len(self.data), self.train_size) // config.batch_size
        #save pics 4 times per epoch
        pic_save_loc = 1 + batch_idxs // 10
...
class gestCondDCGan(DCGAN):
    ...
    def train(self, config):
        ...
        sample_inputs = self.getImageBatch(sample_files, self.resize_height, self.resize_width)
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_images = self.getImageBatch(batch_files, self.resize_height, self.resize_width)
...
#            return tf.nn.sigmoid(h4) #alternate?
    ...
    #given frame # and total # of frames in a sequence
    #return the st index, stweight, end index, end wt in 10 element vec array 
    #used for y vector to encode sequence position
    #so first frame is 0, 1.0, 1, 0; the 8th of 28 frames gives : 2, 0.33, 3, 0.66
    #and the last frame should always be 9,1.0,10,0.0
